"""Generate byte-exact reference vector `append/010-wos-custody-hook-state-transition`.

Authoring aid only. This vector exercises Trellis Core §23 composition with
WOS `custodyHook`: WOS supplies authored dCBOR bytes plus the ADR-0061
idempotency tuple, while Trellis wraps those bytes as an ordinary canonical
append event. The script is not normative; the sibling `derivation.md` is the
human-readable reproduction evidence.
"""
...
OUT_DIR = ROOT / "append" / "010-wos-custody-hook-state-transition"

CASE_ID = "linc_case_01j5d5p0c8e9g2h3j4k5m6n7p8"
RECORD_ID = "linc_prov_01j5d5p1d9f0h3j4k5m6n7p8q9"
LEDGER_SCOPE = f"wos-case:{CASE_ID}".encode("utf-8")
SEQUENCE = 0
TIMESTAMP = ts(1745000010)
EVENT_TYPE = b"wos.kernel.state_transition"
...
def main() -> None:
    ...
    stale_json = OUT_DIR / "input-wos-record.jcs.json"
