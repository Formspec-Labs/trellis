"""Generate byte-exact reference vector `append/004-hpke-wrapped-inline`.

Authoring aid only. Core prose and committed vector bytes are normative; this
script exists to make regeneration and review practical.
"""
...
EPHEMERAL_KEY_FILE = ROOT / "_keys" / "ephemeral-004-recipient-001.cose_key"
PAYLOAD_FILE = ROOT / "_inputs" / "sample-payload-001.bin"
OUT_DIR = ROOT / "append" / "004-hpke-wrapped-inline"

LEDGER_SCOPE = b"test-response-ledger"
SEQUENCE = 0
TIMESTAMP = ts(1745000004)
EVENT_TYPE = b"x-trellis-test/append-hpke-inline"
CLASSIFICATION = b"x-trellis-test/unclassified"
RETENTION_TIER = 0
IDEMPOTENCY_KEY = b"idemp-append-004"
PAYLOAD_NONCE = bytes.fromhex("040404040404040404040404")
...
def main() -> None:
    ...
    payload_ref = build_payload_ref(ciphertext)
    ...
    print()
    print(f"  kid                          = {kid.hex()}")
    print(f"  payload_dek                  = {PAYLOAD_DEK.hex()}")
    print(f"  payload_nonce                = {PAYLOAD_NONCE.hex()}")
    print(f"  recipient_pubkey             = {recipient_pubkey.hex()}")
    print(f"  ephemeral_pubkey             = {ephemeral_pubkey.hex()}")
    print(f"  wrapped_dek                  = {wrapped_dek.hex()}")
